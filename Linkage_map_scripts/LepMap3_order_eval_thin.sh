...
OUTPUT_PATH=$INPUT_PATH/filt_order_all_eval
...
do
INPUT_FILE=$(ls filt_order_all/ordered.${lg}.*_thin)
...
grep "ordered.${lg}.[1-9]" $OUTPUT_PATH/likelihoods.txt |sort -k7 |head -n1 >> $OUTPUT_PATH/list_best_likelihoods.txt
file=$(grep "ordered.${lg}.[1-9]" $OUTPUT_PATH/likelihoods.txt |sort -k7 |head -n1| cut -f1 -d ":" |sed 's/log1/mapped/g')
