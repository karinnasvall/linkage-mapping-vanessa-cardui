...
OUTPUT_PATH=$INPUT_PATH/chr_54_55/OrderMarker_eval_order
...
for lg in ${array_lg[@]}
do
#INPUT_FILE=$(ls filt_order_best_likelihoods/ordered.${lg}.[1-9])

java -Xmx2048m -cp /Applications/lepmap3/bin OrderMarkers2\
 data=$INPUT_DATA\
 evaluateOrder=$INPUT_PATH/chr_54_55/ordered.${lg}.intervals_mod\
 improveOrder=1\
...
sort -k7 $OUTPUT_PATH/likelihoods.txt |awk 'NR%5==1' | sort -t "." -gk2 > $OUTPUT_PATH/list_best_likelihoods.txt
